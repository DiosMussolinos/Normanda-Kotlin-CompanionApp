...
class PrefUtil {
    companion object{
        ...
        fun getTimerState(context: Context): Challenge.TimerState{
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return Challenge.TimerState.values()[ordinal]
        }

        fun setTimerState(state: Challenge.TimerState, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
